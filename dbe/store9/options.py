...
class ModelAdmin(BaseModelAdmin):
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        actions = []
        list_filter = search_fields = list_editable = ()
        list_per_page = 100
        list_select_related = False
        date_hierarchy = None
        # Remove action checkboxes if there aren't any actions available.
        list_display = list_display_links = ['__str__']
        if not actions:
        ...
        media = forms.Media(js=['%s%s' % (settings.MEDIA_PREFIX, url) for url in js])
        ...
        return render_to_response('adminapp/change_list.html', context,
