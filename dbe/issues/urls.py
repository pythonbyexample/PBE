...
urlpatterns = patterns("dbe.issues.views",
    ...
    (r"^update-issue-detail/(?P<pk>\d+)/$", UpdateIssue.as_view(), {}, "update_issue_detail"),
    (r"^issue/(?P<pk>\d+)/$", ViewIssue.as_view(), {}, "issue"),
    (r"^update-comment/(?P<pk>\d+)/$", UpdateComment.as_view(), {}, "update_comment"),
